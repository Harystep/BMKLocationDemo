...
#import "BMKCloudNearbySearchPage.h"
...
/**
 ...
 获取search的回调方法
 */
@interface BMKCloudNearbySearchPage ()<BMKMapViewDelegate, BMKCloudSearchDelegate>
@property (nonatomic, strong) BMKMapView *mapView; //当前界面的mapView
@property (nonatomic, strong) UIButton *customButton;
@end

@implementation BMKCloudNearbySearchPage
...
- (void)createSearchToolView {
    [self createToolBarsWithItemArray:
     @[@{@"leftItemTitle":@"access_key：",
       ...
       @{@"leftItemTitle":@"检索的中心点：",
         @"rightItemText":@"116.282051,40.051231",
         @"rightItemPlaceholder":@"输入检索的中心点"},
       @{@"leftItemTitle":@"周边检索半径：",
         @"rightItemText":@"5",
         @"rightItemPlaceholder":@"输入周边检索半径"}
       ]];
}
...
- (void)setupDefaultData {
    ...
    cloudNearbyInfo.ak = self.dataArray[0];
    //geo table表主键，必选
    cloudNearbyInfo.geoTableId = [self.dataArray[1] intValue];
    //检索的中心点，逗号分隔的经纬度(116.4321,38.76623)，最长不超过25个字符，必选
    cloudNearbyInfo.location = self.dataArray[2];
    //周边检索半径，必选
    cloudNearbyInfo.radius = [self.dataArray[3] intValue];
    [self searchData:cloudNearbyInfo];
}

- (void)searchData:(BMKCloudNearbySearchInfo *)info {
    ...
    //分页数量，默认为10，最多为50，可选
    cloudNearbyInfo.pageSize = info.pageSize;
...
- (void)clickCustomButton {
    BMKCloudNearbyParametersPage *page = [[BMKCloudNearbyParametersPage alloc] init];
        page.searchDataBlock = ^(BMKCloudNearbySearchInfo *info) {
            [self createToolBarsWithItemArray:
             @[@{@"leftItemTitle":@"access_key：",
               ...
               @{@"leftItemTitle":@"geotable表主键：",
                 ...
                 @"rightItemPlaceholder":@"输入表主键"},
               @{@"leftItemTitle":@"检索的中心点：",
                 @"rightItemText":info.location,
                 @"rightItemPlaceholder":@"输入检索的中心点"},
               @{@"leftItemTitle":@"周边检索半径：",
                 @"rightItemText":[NSString stringWithFormat:@"%d", info.radius],
                 @"rightItemPlaceholder":@"输入周边检索半径"}
               ]];
