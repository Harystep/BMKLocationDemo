...
#import "BMKCloudLocalSearchPage.h"
...
/**
 ...
 获取search的回调方法
 */
@interface BMKCloudLocalSearchPage ()<BMKMapViewDelegate, BMKCloudSearchDelegate>
@property (nonatomic, strong) BMKMapView *mapView; //当前界面的mapView
@property (nonatomic, strong) UIButton *customButton;
@end

@implementation BMKCloudLocalSearchPage
...
- (void)setupDefaultData {
    ...
    cloudLocalInfo.ak = self.dataArray[0];
    //geo table表主键，必选
    cloudLocalInfo.geoTableId = [self.dataArray[1] intValue];
    //区域名称(市或区的名字，如北京市，海淀区)，最长不超过25个字符，必选
    cloudLocalInfo.region = self.dataArray[2];
    [self searchData:cloudLocalInfo];
}

- (void)searchData:(BMKCloudLocalSearchInfo *)info {
    ...
    BMKCloudLocalSearchInfo *cloudLocalInfo = [[BMKCloudLocalSearchInfo alloc]init];
    //access_key，最大长度50，必选
    cloudLocalInfo.ak = info.ak;
    //用户的权限签名，最大长度50，可选
    cloudLocalInfo.sn = info.sn;
    //geo table表主键，必选
    cloudLocalInfo.geoTableId = info.geoTableId;
    //检索关键字，最长45个字符，可选
    cloudLocalInfo.keyword = info.keyword;
    //标签，空格分隔的多字符串，最长45个字符，样例：美食 小吃，可选
    cloudLocalInfo.tags = info.tags;
    /**
     ...
     */
    //排序字段，可选： sortby={keyname}:1 升序；sortby={keyname}:-1 降序
    cloudLocalInfo.sortby = info.sortby;
    /**
     过滤条件，可选
     '|'竖线分隔的多个key-value对
     key为筛选字段的名称(存储服务中定义)
    ...
    cloudLocalInfo.filter = info.filter;
    //分页索引，默认为0，可选
    cloudLocalInfo.pageIndex = info.pageIndex;
    //分页数量，默认为10，最多为50，可选
    cloudLocalInfo.pageSize = info.pageSize;
    //区域名称(市或区的名字，如北京市，海淀区)，最长不超过25个字符，必选
    cloudLocalInfo.region = info.region;
    /**
     ...
     成功返回YES，否则返回NO
     */
    BOOL flag = [cloudSearch localSearchWithSearchInfo:info];
    if(flag) {
        NSLog(@"本地云检索成功");
    } else {
        NSLog(@"本地云检索失败");
...
- (void)clickCustomButton {
    BMKCloudLocalParametersPage *page = [[BMKCloudLocalParametersPage alloc] init];
    page.searchDataBlock = ^(BMKCloudLocalSearchInfo *info) {
        [self createToolBarsWithItemArray:
         @[@{@"leftItemTitle":@"access_key：",
